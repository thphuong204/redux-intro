...
const RootComponent = (props) => {
  ...
  const addProductToCart = (productObject) => {
    const newProductsCart = cart.products.map((cartProduct) => {
      if(cartProduct.id === productObject.id) {
        cartProduct.qty += 1;
        cartProduct.price += productObject.price
      }
      return cartProduct;
    });

    let newTotalPrice = 0;
      newProductsCart.forEach(object => {
        newTotalPrice +=object.price
      })
    
    setCart({products:newProductsCart, totalPrice: newTotalPrice})
  }
  ...
  const removeProductFromCart = (productObject) => {
    const newProductsCart = cart.products.map((cartProduct) => {
      if(cartProduct.id === productObject.id) {
        cartProduct.qty = Math.max(cartProduct.qty-1,0);
        cartProduct.price = Math.max(cartProduct.price-productObject.price,0);
      }
      return cartProduct;
    });
